...
def index(request):
    ...
    #合计
    sqlSum = "select b.shopid,b.shopname,SUM(b.qtyz) AS qtyz,SUM(b.qtyl) AS qtyl,(SUM(b.qtyl)/SUM(b.qtyz)) AS zhonbi,(select count(distinct zhonbi) from KNegativestock a where a.zhonbi <= b.zhonbi) AS mingci from KNegativestock AS b WHERE stockdate BETWEEN '"+start+"' AND '"+today+"' GROUP BY shopid order by shopid"
    print(sqlSum)
    cur = conn.cursor()
    cur.execute(sqlSum)
