...
def query(date):
    ...
    baselist = Kshopsale.objects\
        .values('shopid', 'sdate', 'salevalue', 'salegain', 'tradenumber', 'tradeprice',
...
@cache_page(60*10,key_prefix='daily_group_operate')
@csrf_exempt
def index(request):
...
def countSum(sumList,days):
    ...
    for key in sumList.keys():
        ...
        if float(sum["month_salegainold"]) > 0:
            sum["month_salegain_ynygrowth"] = str("%0.2f" % ((float(sum["month_salegain"])-float(sum["month_salegainold"]))*100.0/float(sum["month_salegainold"])))+"%"
        else:
...
def findMonthEstimate(shopids):
    ...
    karrs.setdefault("groupid__lte", "70")
